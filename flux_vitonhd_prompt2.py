...
from vitonhddata import VitonHDDataset,VitonHDDatasettestimage
...
def main(args):
    ...
    if args.validation_prompt is not None:
        ...
        testdata = VitonHDDatasettestimage(
            dataroot_path=args.instance_data_dir,
            phase='test'
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if args.cache_latents:
                    ...
                    with offload_models(vae, device=accelerator.device, offload=args.offload):
                        pixel_values = batch["pixel_values"].to(dtype=vae.dtype)  # (B, 3, H, W)
                        model_input = vae.encode(pixel_values).latent_dist.mode()  # (B, C, h, w)
                        # print(model_input.shape)
                        model_input = Flux2KleinPipeline._patchify_latents(model_input)  # (B, N, D)
                        # print(model_input.shape)
                        ...
                        for cond_img in batch["cond_pixel_values"]:
                            ...
                            # print(latent.shape)
                            packed = Flux2KleinPipeline._patchify_latents(latent)  # (B, N, D)
                        ...
                        # Concatenate along sequence dimension: (B, N_cond * N, D)
                        # print(packed_latents_list.shape)
                        # cond_model_input = torch.cat(packed_latents_list, dim=2)  # 注意是 dim=1!
                        # print(cond_model_input.shape)
                        # cond_model_input = (cond_model_input - latents_bn_mean) / latents_bn_std

                        # === 构造 image IDs ===
                        # For main image: (B, N, 2)
                        model_input_ids = Flux2KleinPipeline._prepare_latent_ids(model_input).to( device=model_input.device)  # (B, N, 2)
                        #cond_model_input_ids = Flux2KleinPipeline._prepare_image_ids(packed_latents_list).to( device=model_input.device)
                        cond_input_list=[]
                        ...
                        cond_model_input_ids = Flux2KleinPipeline._prepare_image_ids(cond_input_list).to( device=model_input.device)
                        cond_model_input_ids = cond_model_input_ids.view(
                            packed_latents_list[0].shape[0], -1, model_input_ids.shape[-1]
                        )
# 遍历6个tensor，每个都执行原unsqueeze(0)操作，保持单tensor处理逻辑一致
                        
                    
                        # print(cond_model_input_ids.shape)
                #     with offload_models(vae, device=accelerator.device, offload=args.offload):
                #         pixel_values = batch["pixel_values"].to(dtype=vae.dtype)
                #         cond_model_input=[]
                #         packed_latents = []
                #         for cond_img in  batch["cond_pixel_values"]:
                #             cond_img_values = cond_img.to(dtype=vae.dtype)
                #             cond_img_values = vae.encode(cond_img_values).latent_dist.mode()
                #             cond_model_input.append(cond_img_values) 
                #             packed = Flux2KleinPipeline._patchify_latents(cond_img_values)
                #             print(packed.shape)
                #             packed = packed.squeeze(0)  # (1024, 128) - remove batch dim
                #             packed_latents.append(packed)
                #             # image_latents = torch.cat(packed_latents, dim=0)  # (N*1024, 128)
                #             # cond_img_values = (cond_img_values - latents_bn_mean) / latents_bn_std

                #             # packed_latents.append(cond_img_values)
                ...
                # # cond_model_input = torch.cat(packed_latents, dim=0)  # (N*1024, 128)
                # # cond_model_input = cond_model_input.unsqueeze(0)  # (1, N*1024, 128)
                # # print(cond_model_input.shape)
                # # cond_model_input = (cond_model_input - latents_bn_mean) / latents_bn_std
                ...
                packed_latents_list
                packed_latents = []
                for latent in packed_latents_list:
                    # latent: (1, 128, 32, 32)
